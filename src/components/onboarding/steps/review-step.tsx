...
export function ReviewStep({
  ...
  const sections: Array<{
  ...
  }> = [
    {
      title: 'Account Type',
      data: {
        'Account Type': accountType === 'employee-group-life' ? 'Employee Group Life' : accountType,
      },
    },
    {
      title: 'Policy Plan',
  ...
  if (accountType === 'individual' || accountType === 'employee-group-life') {
    sections.push({
      title: 'Personal Info',
      data: {
        'Full Name': `${onBoardingData?.title || ''} ${onBoardingData?.first_name || ''} ${onBoardingData?.last_name || ''}`.trim(),
        'Gender': onBoardingData?.gender,
        'Date of Birth': onBoardingData?.date_of_birth ? new Date(onBoardingData.date_of_birth).toLocaleDateString() : null,
  ...
  if (accountType === 'corporate') {
    sections.push({
      title: 'Bank Details',
      data: {
        "Director's BVN": onBoardingData?.director_bvn_number ? '••••••' + onBoardingData.director_bvn_number.slice(-4) : 'Not provided',
        "Director's Bank": onBoardingData?.director_bank_name,
        "Director's Account": onBoardingData?.director_bank_acct_number,
        'TIN': onBoardingData?.director_tax_identification_number || 'Not provided',
      },
    });
  } else {
    sections.push({
      title: 'Bank Details',
      data: {
        'BVN': onBoardingData?.bvn_number ? '••••••' + onBoardingData.bvn_number.slice(-4) : 'Not provided',
  ...
  sections.push({
    ...
    data: {
      ...
      'ID Number': onBoardingData?.identity_card_number || (onBoardingData?.national_identification_number ? '•••••••••••' : null),
    },
  });
  ...
  sections.push({
    title: 'Documents',
    data: {
      'Identity Card': onBoardingData?.identity_card || onBoardingData?.director_identity_cards ? '✓ Uploaded' : 'Not uploaded',
      'CAC Document': onBoardingData?.cac_document ? '✓ Uploaded' : (accountType === 'corporate' ? 'Not uploaded' : 'N/A'),
      'Passport': onBoardingData?.passport_photograph || onBoardingData?.director_passport_photograph ? '✓ Uploaded' : 'Not uploaded',
      'Signature': onBoardingData?.scanned_signature ? '✓ Uploaded' : (accountType === 'individual' || accountType === 'employee-group-life' ? 'Not uploaded' : 'N/A'),
      'NIN Document': onBoardingData?.nin_document ? '✓ Uploaded' : (onBoardingData?.identity_card_type === 'National Identity Number' ? 'Not uploaded' : 'N/A'),
    },
  });

  // Beneficiaries (for employee group life)
  if (accountType === 'employee-group-life' && onBoardingData?.beneficiaries) {
    const beneficiariesData: any = {};
    onBoardingData.beneficiaries.forEach((b: any, index: number) => {
      beneficiariesData[`Beneficiary ${index + 1}`] = `${b.first_name} ${b.last_name} (${b.percentage_allocation}%)`;
    });
    sections.push({
      title: 'Beneficiaries',
                  ...
                  <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(section.data).map(([key, value]) => (
                      <div key={key} className={cn(
                        "space-y-1",
                        typeof value === 'string' && value.includes('Not') && "text-amber-600"
                      )}>
