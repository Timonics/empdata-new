...
interface PersonalInfoSectionProps {
  ...
  onValidationChange?: (isValid: boolean) => void; // Optional callback for parent validation
}
...
export function PersonalInfoSection({
  ...
  // Determine which fields are required based on account type
  const getRequiredFields = () => {
    ...
    // For employee-group-life, we might have different requirements
    // For now, keep the same as individual
...
//     data.email_address === data.confirm_email_address;

//   const phoneMatch =
//     data?.phone_number &&
//     data?.confirm_phone_number &&
//     data.phone_number === data.confirm_phone_number;

//   const hasValidDateOfBirth = data?.date_of_birth instanceof Date;

//   return allFieldsPresent && emailMatch && phoneMatch && hasValidDateOfBirth;
// };
  ...
  // Determine if this component should be rendered based on account type
  const shouldRender = isIndividual || isEmployeeGroupLife;

  if (!shouldRender) {
    return null; // Don't render for corporate
  }

  // Get header text based on account type
  const getHeaderText = () => {
    if (isEmployeeGroupLife) {
  ...
  return (
    <div className="space-y-6">
      ...
      {isEmployeeGroupLife && (
        <div className="mb-4">
          <h4 className="text-md font-medium text-gray-900">{getHeaderText()}</h4>
          <p className="text-sm text-gray-500">
            Please provide your personal details for group life enrollment
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 overflow-auto">
        ...
        <div className="space-y-2 mb-4 pr-1 md:col-span-2">
          <label className="text-sm font-medium text-gray-700">
