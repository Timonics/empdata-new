...
import { useApproveEmployeeRegistration, useEmployeeRegistrations, useRejectEmployeeRegistration, useSendEmployeeInvitation } from "@/hooks/queries/useGroupLifeEmployees";
import { formatDistanceToNow } from "date-fns";
import { RegistrationFilters } from "@/types/grouplife.types";
import { IndividualDrawer } from "./individual-drawer";
import { IndividualStats } from "./individual-stats";
...
export function IndividualRegistrations() {
  ...
  const { data, isLoading, refetch, isFetching } = useEmployeeRegistrations(filters, currentPage);
      ...
      cell: (item: any) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{item.email_address}</p>
          </div>
        </div>
      ),
    },
      ...
      cell: (item: any) => (
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">ID: {item.company_id}</p>
          </div>
        </div>
      ),
    },
